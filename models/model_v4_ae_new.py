...
from .utils import latest_epoch
...
class Model_v4_AE:
    def __init__(self, config):
        ...
        self.converter = nn.build_architecture(
            architecture_descr['converter'], custom_objects_code=config.get('custom_objects', None)
        )
        ...
        epoch = latest_epoch(Path(self.checkpoint))
        ...
        self.converter.compile(optimizer=self.opt, loss='mean_squared_error')

        self.load_encoder(self.checkpoint + '/encoder_{:05d}.h5'.format(epoch))
        self.load_decoder(self.checkpoint + '/decoder_{:05d}.h5'.format(epoch))
    ...
    def save_weights(self, path, step):
        ...
        self.converter.save(str(path.joinpath("converter_{:05d}.h5".format(step))))
    ...
    def make_fake(self, features):
        ...
        latent_input = self.converter(tf.concat([_f(features), latent_input], axis=-1))
    ...
    def decode(self, features, z):
        z_converted = self.converter(tf.concat([_f(features), z], axis=-1))

        return self.decoder(tf.concat([_f(features), z_converted], axis=-1))
